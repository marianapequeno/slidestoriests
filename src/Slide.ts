...
export default class Slide {
  ...
  show(index: number) {
    this.index = index;
    this.slide = this.slides[this.index];
    localStorage.setItem("activeSlide", String(this.index)); //Salva no localStorage onde parou
  ...
  prev() {
    ...
    const prev = this.index > 0 ? this.index - 1 : this.slides.length - 1; //Ou 0 (zero) para não voltar
    this.show(prev);
  }
  next() {
    if (this.paused) return;
